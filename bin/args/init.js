...
var getConf = function () {
    ...
    target.pop();
    return path.join(target.join('/'), 'resources/srcy.conf.js');
};
...
module.exports = (function () {
    ...
    if (!isForced() && fs.existsSync('srcy.conf.js')) {
        log.warn('srcy.conf file already exists');
        log.info('use "srcy init -f" to over-write it with a blank conf file');
        process.exit(1);
    }

    fs.writeFileSync('srcy.conf.js', conf, 'utf8');

    log.success('blank srcy.conf.js file created');
